...
## 后续：`speakers at` 的退化窗口（诊断已闭环，代码改动推迟）

Hand 重跑 08-05 后报「id=16 的 participants 为空，因为该窗口没有稳定在场者，可接受」。
**理由不成立**：那条 episode 的 `started_at == ended_at == 1785907858.0` —— **零长度窗口**。
在场判定是重叠 `t.started_at < to AND t.ended_at > from`，两端相同就要求某个 turn
**严格跨过这一个瞬间**，于是恒空。查询从来没机会去看那段时间有谁。

生产实测三种窗口（这也是这类问题的通用定位法）：

| 查询 | present | unbound_turns |
|---|---|---|
| 原样 13:30–13:30 | `[]` | 0 |
| 前后各放宽 5 分钟 | `[]` | **1** |
| `--date 2026-08-05` | spk_002(13) / spk_007(7) / spk_001(1) / spk_003(1) | 15 |

**放宽后 unbound 从 0 变 1 是决定性证据**：该时段确实有一条人声（短到没绑定），
所以空不是「没人」而是「查不了」。规律：**任何 `start_clock == end_clock` 的 episode
都会拿到空 participants，与那段时间实际有谁完全无关**。

处置：Hand 已在消费侧用 ±3min 兜底（一分钟精度的时钟本就代表区间而非点），
并明确表示不希望我为此插队部署。**Mind 侧待办（商定推迟，非遗忘）**：
给 `speakers at` 加 `warning` 字段，`ended_at <= started_at` 时明说「窗口退化、结果不可解读」——
下次改 speakers 时顺手加，否则同一个坑会换个人再踩。

佩戴者补全（窗口内有 unbound turn 且当天佩戴者活跃时把 participants 补成 `["我"]`）
属于推断，我建议不做、Hand 同意不做。

## 收尾：四天重跑完成，Mind 侧独立核验（2026-08-09）

Hand 跑完 ①②③ 并按 id 删净旧数据。以下是**我独立查生产**的结果，不是转述：

```
episodes 128 | started_at=0 → 0 | FTS 128 行
FTS 孤儿（索引有、主表无）  : 0
反向漏网（主表有、索引无）  : 0
id <= 14 残留               : 0
memories 251 / speaker_turns 9142 / speakers 14 / schema 8
```

**两个方向都查了**：FTS 无孤儿只证明「删干净」，若新插的 128 条有哪条没进索引，
搜索会**静默漏掉**它——比幽灵结果更隐蔽，因为没人会注意到搜不到的东西。
两向皆 0，说明 `delete_episodes`「先删 FTS 再删主表」的顺序在真实数据上有效，
写入路径的索引维护也一致。`speaker_turns` 一条未动，`memories` 148 → 251（新 episode 蒸馏所得）。

成果：85 个字母假名单没了，participants 现在是 `['spk_001','spk_003','我','spk_008']`
这样的真实身份，由 `speakers at` 产出；episode 时钟不再是模型猜的。

## 这次跨会话协作里守住的两条线（值得复用）

1. **权限不洗白**：同伴转述的 owner 批准不解锁本会话被拒的动作，回到自己的用户要授权；
   对称地，同伴也拒绝了「因为你被挡我替你做」。
2. **删真实数据双人核对**：Hand 给出待删 14 个 id，我独立查生产 `WHERE started_at=0`
   按天分组比对，逐天一字不差才动手。不可逆操作值得两个人各查一遍。
