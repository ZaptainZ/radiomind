...
> ⚠️ **历史结果（gpt-4o 跑，2026-04-20）。** 这是历史 n=100 artifact，**不是**
> 经过 V8.x / 诊断 / closure 工作后的 current-main 基线。要作为当前 LoCoMo 分数
> 对外引用前必须重跑。

| 系统 | 分数 |
|---|---:|
| Mem0 v3                                          | 0.916 |
| **RadioMind（历史 gpt-4o 跑，2026-04-20）**      | **0.890** |
| MemMachine                                       | 0.917 |

在那次历史跑上比 Mem0 落后 2.6 pt 之内。剩余 gap 是对话特化的（指代消解、说话人追踪），后续迭代空间。
